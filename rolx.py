...
# From https://github.com/Lab41/Circulo/blob/master/circulo/algorithms/rolx.py
...
def recursive_feature(G, f, n):
    ...
    """
    return np.array(recursive_feature_array(G, f, n))
...
def approx_linear_solution(w, A, threshold=1e-15):
    ...
    result = norm_residual <= threshold
...
def vertex_features(g):
    ...
    features = [degree, egonet, egonet_out]  # Assuming these are predefined feature functions
    V = np.zeros((num_rows, 16*len(features)))
...
def add_col(V, b, insert_col):
    ...
    V[:, insert_col] = b.reshape(-1)  # Reshape b to ensure it's a 1D array
    return V
...
def addNodeToIGraph(Graph, video_dict_id, dict_to_graph, graph_to_dict, current_graph_counter):
...
def load_graph_igraph(fname, fname_egonet=None):
    ...
    for video_node in video_dict_list.values():
        ...
        current_graph_counter = addNodeToIGraph(Graph, video_dict_id, dict_to_graph, graph_to_dict, current_graph_counter)
        ...
        for related_dict_id in related_dict_ids:
            current_graph_counter = addNodeToIGraph(Graph, related_dict_id, dict_to_graph, graph_to_dict, current_graph_counter)
